...
class Quote(Base):
    ...
    author_id = Column(Integer, ForeignKey('author.id')) # Many quotes, one author
    tags = relationship(
        'Tag',
        secondary = 'quote_tag',
        lazy = 'dynamic',
        backref = "quote"
    ) # Many-to-many for quote/tag
...
class Author(Base):
    ...
    bio = Column('bio', Text())
    quotes = relationship('Quote', backref = 'author') # One author, many quotes
